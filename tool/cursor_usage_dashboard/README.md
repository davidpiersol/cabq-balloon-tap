...
1. From the repo root: **Tasks: Run Task** → **Cursor Usage Dashboard: start (demo)** (or run `CURSOR_USAGE_DEMO=1 streamlit run app.py` in `tool/cursor_usage_dashboard`).
...
```cron
*/15 * * * * cd /Users/you/Developer/balloon-tap/tool/cursor_usage_dashboard && /usr/bin/env DATABASE_URL="postgresql://..." /usr/bin/python3 collector.py >> collector.log 2>&1
```
...
- App entrypoint: `tool/cursor_usage_dashboard/app.py`
- Python deps: `tool/cursor_usage_dashboard/requirements.txt`
